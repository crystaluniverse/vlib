...
import vfs
import freeflowuniverse.crystallib.vfs.vfsnested
import os
...
fn main() {
	...
	mut vfs1 := vfs.new_local_vfs('/tmp/test_nested_vfs/vfs1') or { panic(err) }
	mut vfs2 := vfs.new_local_vfs('/tmp/test_nested_vfs/vfs2') or { panic(err) }
	mut vfs3 := vfs.new_local_vfs('/tmp/test_nested_vfs/vfs3') or { panic(err) }
