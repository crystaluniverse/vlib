...
fn (mut factory VDocFactory) process_dir(dir_path string) ! {
	...
	for item in items {
		...
		if os.is_dir(full_path) {
			...
			factory.process_dir(full_path) or {
				return error('Failed to process directory: ${err}')
			}
			continue
		}
...
factory.process_dir('/Users/despiegk1/code/github/freeflowuniverse/crystallib/crystallib') or {
	return error('Failed to process root directory: ${err}')
}
